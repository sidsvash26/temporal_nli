...
# This code is origially taken from huggingface library (license copied above) from below link:
# https://github.com/huggingface/transformers/blob/master/src/transformers/trainer.py
# Code modified slightly for saving checkpoints by: Siddharth Vashishtha
import json
import logging
import math
import os
...
class Trainer:
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    if self.args.save_steps > 0 and self.global_step % self.args.save_steps == 0:
                        ...
                        if self.is_world_master():
                            ...
                            #print(f"saving optimizer_states --")
    ...
    def evaluate(
        ...
        ## add predictions to checkpoint folder#
        output_dir = os.path.join(self.args.output_dir, f"{PREFIX_CHECKPOINT_DIR}-{self.global_step}")
        output_dir = output_dir if output_dir is not None else self.args.output_dir
        os.makedirs(output_dir, exist_ok=True)
        logger.info("Saving model checkpoint predictions to %s", output_dir)
        torch.save(output, os.path.join(output_dir, "eval_predictions.pkl"))
